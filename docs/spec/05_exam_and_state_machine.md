...
## 試験進行・状態遷移仕様（Exam / Attempt / Section）
...
- `exam_version_sections.position` の昇順で実施
- 受験者は順序を選択できない
- セクションを飛ばすことは禁止
...
- 各セクション内の問題は `exam_version_questions.position` 順
- 出題順は Attempt 開始時に確定

### 6.2 戻り操作

- 同一セクション内での「前の問題に戻る」可否は UI 設計次第
- ただし、**セクションを跨いで戻ることは禁止**
...
- Attempt 開始時点で以下を **全て作成**する
  ...
  - `attempt_section_timers`（セクション残り時間）
- 「セクション開始時に順次作成」などの遅延作成は行わない
