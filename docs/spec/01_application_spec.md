...
- 2026-01-11: 試験定義（Exam / ExamVersion / Section）の運用要件を明文化
...
- 2026-01-11: 来社枠・来社割当をスコープ外へ変更
- 2026-01-11: 試験構成の呼称を「セクション」から「セクション」に統一
...
- ExamVersion ごとにセクション構成と制限時間を設定できる
- 必須セクション
  - VERBAL / NONVERBAL / ENGLISH / STRUCTURAL
- セクション順序は position で固定し、重複は不可
- 制限時間は section ごとに保持する

#### 出題割当（必須）

- Staff は DRAFT の ExamVersion に対して出題割当（question / section / position / points）を設定できる
- PUBLISHED / ARCHIVED の ExamVersion は出題割当の編集不可
- 同一 ExamVersion 内で question は重複不可
- 同一 ExamVersion + section 内で position は重複不可

#### 試験定義の運用フロー（ガイド）

1. 試験マスタを新規作成（Exam）
2. 試験バージョンを追加（ExamVersion、DRAFT）
3. セクション構成と制限時間を設定（必須4セクション）
4. 出題割当を設定（question / section / position / points）
...
- 不正行為の自動検出・自動失格判定
- 来社枠・来社割当（visit_slots / candidate_slot_assignments）の運用
