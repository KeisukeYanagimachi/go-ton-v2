...
- 2026-01-11: 来社枠・来社割当のUX要件を明文化
- 2026-01-11: 候補者管理（Staff）のUX要件を明文化
- 2026-01-11: 来社枠・来社割当をスコープ外として削除
...
- 操作対象は DRAFT のみ
- 出題割当の入力は「対象バージョン → セクション → 問題」の順で選択
- 問題は検索可能、選択中の問題をプレビュー表示
- 割当一覧はセクション単位でグルーピングし、件数を明示
- エラー/成功メッセージは出題割当セクション内に表示
- 問題が未登録の場合は「問題が登録されていません」と明示し選択を無効化
- 未分類（セクション未設定）の問題は出題割当の候補に表示しない
...
- 何をしたいか: Candidate に受験票を配布したい
- 何を知らないか: 受験票コード、QRの有無
- 不安: 正しい受験票を渡せているか
...
- 新しい受験票コードとQRコードを結果として表示する
- 旧ticketが無効化されたことを明示する
- **印刷ボタン**を用意し、紙配布ができる
...
- 何をしたいか: 候補者情報を登録・確認したい
- 何を知らないか: 対象候補者が既に登録済みか、どの情報が必須か
- 不安: 間違った候補者を更新してしまう
...
- 一覧（左）と詳細（右）を分離し、選択中の候補者が分かる構成
- 一覧は「氏名 / Candidate ID」で検索できる
- 詳細は **full_name / birth_date** を必須表示する
- 登録フォームは必須項目を明確にし、未入力時は保存できない
- 登録/更新結果のメッセージはフォーム近くに表示する
- 候補者の作成・編集は ADMIN 権限であることを明示する
...
- QRコード読み取りで ticket_code を入力できる
- QR読み取りができない場合は **手動入力**で代替できる
- 成功後は「試験開始」画面へ遷移する
- 失敗時はフォーム直下にエラーを表示する
